...
export default function App() {
    ...
    const fetchSafe = async <T,>(url: string, fallbackValue: T): Promise<T> => {
      ...
      try {
        ...
        const timeoutId = setTimeout(() => {
          console.warn(`[App Init] [FetchSafe] Timeout of 6s reached for: ${url}. Aborting request.`);
          controller.abort();
  ...
  useEffect(() => {
    ...
    // Safety fallback timeout: closes loading screen unconditionally in 8 seconds max
    const safetyTimeoutId = setTimeout(() => {
      console.warn("[App Init] Safety fallback timeout (8 seconds) triggered! Closing loading screen unconditionally.");
      setLoading(false);
    }, 8000);
